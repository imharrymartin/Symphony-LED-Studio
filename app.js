...
document.getElementById('btn-arrange-record').addEventListener('click', () => {
    ...
    if (arranger.isRecording) {
        ...
        btn.style.boxShadow = '0 0 20px #ff3366';
        arranger.playheadMs = 0;
...
function updateBeatTrackLabel(state) {
    ...
    if (state && state.track_id) {
        const prefix = state.is_playing ? '▶' : '⏸';
        beatTrackLbl.textContent = `${prefix} ${state.track_name || 'Unknown'} — ${state.artist_name || ''}`;
        beatTrackLbl.style.color = state.is_playing ? '#1DB954' : '#8b9bb4';
    } else {
        beatTrackLbl.textContent = 'Nothing detected — open Spotify and play a song.';
        beatTrackLbl.style.color = '#8b9bb4';
    }
}

btnBeatStart?.addEventListener('click', async () => {
    beatStatus.textContent = '⌛ Loading beat analysis from Spotify...';
    beatStatus.style.color = '#8b9bb4';
    try {
        ...
        const data = await res.json();
        if (data.error) {
            beatStatus.textContent = `❌ ${data.error}`;
            beatStatus.style.color = '#ff3366';
            return;
        }
        beatSyncActive = true;
        btnBeatStart.style.display = 'none';
        btnBeatStop.style.display  = 'inline-block';
        beatStatus.textContent = '✅ Beat Sync running — lights are locked to the music!';
        beatStatus.style.color = '#1DB954';

        // Stop the regular audio sync if it was running
        if (btnAudioStart.style.display === 'none') btnAudioStop.click();
        if (btnScreenStart.style.display === 'none') btnScreenStop.click();
    } catch (e) {
        beatStatus.textContent = '❌ Server error — is it running?';
        beatStatus.style.color = '#ff3366';
    }
});
